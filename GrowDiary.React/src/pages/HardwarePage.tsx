...
function HardwarePage() {
  ...
  async function saveHardware(event: FormEvent<HTMLFormElement>) {
    ...
    if (!draft.name.trim()) {
      setError('Bitte Geraetename eingeben.')
      return
    }
    ...
    try {
      if (editingId) {
        ...
        setMessage('Hardware angelegt. Entity-Verknuepfung erfolgt separat im Home-Assistant-Mapping.')
      }
      setEditingId(null)
      setDraft(createDraft())
      await load()
  ...
  async function updateHardwareStatus(item: HardwareItemDto, status: HardwareItemStatus) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Hardwarestatus konnte nicht geaendert werden.')
    } finally {
      setSaving(null)
    }
  }
  ...
  return (
    <V1Page eyebrow="Ops" title="Sensoren" subtitle="Physisches Inventar, Zeltzuordnung und Wartungsstatus. HA-Entities werden separat unter Home Assistant gemappt.">
      ...
      <section className="v1-kpi-grid">
        ...
        <V1Card><span className="v1-card-kicker">Inventar</span><h2>{hardware.length}</h2><p>Geraete</p></V1Card>
        <V1Card><span className="v1-card-kicker">Zelte</span><h2>{tents.length}</h2><p>Zuordnung moeglich</p></V1Card>
        <V1Card tone={plannedMaintenance.length + plannedCalibration.length > 0 ? 'warn' : 'ok'}><span className="v1-card-kicker">Pflege</span><h2>{plannedMaintenance.length + plannedCalibration.length}</h2><p>Wartung/Kalibrierung faellig</p></V1Card>
      </section>

      <V1Tabs<Tab> label="Sensoren Bereich" active={tab} onChange={setTab} items={[{ value: 'status', label: 'Status', meta: `${sensors.length} Sensoren` }, { value: 'inventory', label: 'Inventar', meta: `${hardware.length} Geraete` }, { value: 'maintenance', label: 'Wartung', meta: `${plannedMaintenance.length + plannedCalibration.length} offen` }, { value: 'mapping', label: 'Mapping', meta: 'HA getrennt' }]} />

      {loading ? <V1Empty title="Lade Sensoren..." /> : tab === 'status' ? (
        <V1Section title="Sensorstatus">
          {sensors.length === 0 ? <V1Empty title="Noch keine Sensor-Hardware" text="Lege pH-, EC-, ORP-, DO- oder Klima-Sensoren im Inventar an." action={<V1Button variant="primary" onClick={() => setTab('inventory')}>Inventar oeffnen</V1Button>} /> : (
            <div className="ops1b-sensor-grid">
              {sensors.map((item) => <HardwareCard key={item.id} item={item} saving={saving === `hardware-${item.id}`} onStatus={updateHardwareStatus} onEdit={startEdit} />)}
            </div>
          )}
        </V1Section>
      ) : tab === 'inventory' ? (
          ...
          <V1Section title="Inventar">
            {hardware.length === 0 ? <V1Empty title="Keine Hardware angelegt" /> : (
              <div className="ops1b-inventory-grid">
                {hardware.map((item) => <HardwareCard key={item.id} item={item} saving={saving === `hardware-${item.id}`} onStatus={updateHardwareStatus} onEdit={startEdit} />)}
              </div>
            )}
          </V1Section>

          <V1Section title={editingId ? 'Sensor oder Geraet bearbeiten' : 'Sensor oder Geraet anlegen'}>
            <form className="ops1b-form" data-audit="hardware-edit-form" onSubmit={(event) => void saveHardware(event)}>
              <div className="ops1b-form-grid">
                ...
                <V1Field label="Kritikalitaet"><select value={draft.criticality} onChange={(event) => setDraft((current) => ({ ...current, criticality: event.target.value as HardwareItemCriticality }))}>{criticalityOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select></V1Field>
        ...
        <V1Section title="Wartung & Kalibrierung">
          {plannedMaintenance.length + plannedCalibration.length === 0 ? <V1Empty title="Keine faelligen Sensoraufgaben" /> : (
            <div className="ops1b-inventory-grid">
        ...
        <V1Section title="Home-Assistant-Mapping">
          <V1Card>
            <span className="v1-card-kicker">Getrennte Zustaendigkeit</span>
            <h2>Inventar ist nicht Entity-Mapping</h2>
            <p>Sensoren werden hier als Hardware gefuehrt. Die konkrete Home-Assistant-Entity wird im HA-Mapping am Zelt gepflegt.</p>
            <div className="v1-action-row"><V1LinkButton to="/home-assistant" variant="primary">Entity im HA-Mapping verknuepfen</V1LinkButton></div>
          </V1Card>
        </V1Section>
      )}
    </V1Page>
...
function HardwareCard({ item, saving, onStatus, onEdit }: { item: HardwareItemDto; saving: boolean; onStatus: (item: HardwareItemDto, status: HardwareItemStatus) => void; onEdit: (item: HardwareItemDto) => void }) {
  ...
  return (
    <V1Card tone={tone}>
      ...
      <p>{item.haEntityId ? `HA: ${item.haEntityId}` : 'HA-Entity im HA-Mapping verknuepfen.'}</p>
...
function EventCard({ title, meta }: { title: string; meta: string }) {
  return <V1Card tone="warn"><span className="v1-card-kicker">Faellig</span><h2>{title}</h2><p>{meta}</p></V1Card>
}
